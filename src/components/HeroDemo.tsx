...
function JobIcon({ kind }: { kind: HeroJob["icon"] }) {
  switch (kind) {
    case "outbound":
      return (
        <svg viewBox="0 0 24 24" fill="none" aria-hidden>
...
export function HeroDemo() {
  ...
  const job = HERO_JOBS[activeIndex] ?? HERO_JOBS[0];

  return (
    <section className="hero" aria-labelledby="hero-title">
      <div className="hero-copy">
        <p className="eyebrow">A point of view for Qorvo</p>
        <h1 id="hero-title">Every agent has a computer.</h1>
        <p className="hero-intro">
          They keep the work around engineering and customer teams moving.
